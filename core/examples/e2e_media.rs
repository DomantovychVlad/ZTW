//! НАСКРІЗНЕ МЕДІА: реальний екран керованого → H.264 → крізь ЖИВУ зашифровану сесію
//! (server-mediated P2P str0m + PAKE) → пульт збирає чанки й валідує H.264-потік.
//!
//! host(=offerer) захоплює основний монітор (WGC), кодує в H.264 (Media Foundation),
//! ріже access units на чанки (core::media) і шле їх datachannel'ом, який вже піднято
//! через сервер і захищено PAKE-сесією. controller(=answerer) збирає кадри назад і
//! перевіряє, що це валідний Annex-B H.264 із keyframe (SPS).
//!
//! Windows-only (захоплення/кодек ОС). Сервер має бути піднятий. Запуск:
//!   cargo run -p zortilwatch-core --example e2e_media

#[cfg(not(windows))]
fn main() {
    println!("e2e_media: лише Windows (WGC-захоплення + Media Foundation)");
}
...
mod imp {
    ...
    fn recv_one(rtc: &mut Rtc, sock: &UdpSocket, my_addr: SocketAddr) {
        ...
        match sock.recv_from(&mut buf) {
            ...
            // ConnectionReset (Windows 10054) — наслідок ICMP port-unreachable, коли пір
            // завершив сесію й закрив сокет. Не фатально: сокет придатний далі.
            Err(ref e)
    ...
    /// Спільне: піднята й ПІДТВЕРДЖЕНА сесія (live rtc/sock/chan + сесійний ключ).
    struct Established {
        rtc: Rtc,
        sock: UdpSocket,
        my_addr: SocketAddr,
    ...
    /// Драйвити str0m + PAKE до підтвердження; ПОВЕРНУТИ живий стан (канал відкритий).
    ...
    // ── HOST = offerer: захоплення + кодек + стрім ─────────────────────────────
    fn run_host(url: &str, id: &str, secret: &str, ready: Arc<Barrier>) -> Result<usize, String> {
        ...
        println!("HOST: session confirmed ✓ — захоплюю екран, кодую і стрімлю {N_FRAMES} кадрів…");

        // Єдиний неблокувальний цикл: захоплюємо+кодуємо нові кадри, женемо чанки каналом
        // і ПОСТІЙНО качаємо str0m (інакше пауза на захопленні може підвісити з'єднання).
        let (cap, rx) = capture::start_primary().map_err(|e| e.to_string())?;
        let mut enc: Option<H264Encoder> = None;
        let mut chunker = Chunker::new(DEFAULT_MAX_PAYLOAD);
        let mut sealer = StreamSealer::new(&est.key, MEDIA_LABEL); // E2E-шифрування медіа
        ...
        loop {
            if encoded >= N_FRAMES && queue.is_empty() {
                ...
                if Instant::now() - t > Duration::from_secs(2) {
                    break; // усе надіслано й трохи пофлашили
                }
            }
            if Instant::now() > deadline {
            ...
            // Наступний кадр (неблокувально) -> кодек -> чанки.
            if encoded < N_FRAMES {
            ...
            // Вивантажуємо чанки (з урахуванням backpressure SCTP).
    ...
    fn run_controller(
        ...
        let opener = StreamOpener::new(&est.key, MEDIA_LABEL); // розшифрування медіа
        let mut chan_opt = Some(est.chan);
        let mut inbox = Vec::new();
    ...
    pub fn run() {
        ...
        match (&hr, &cr) {
            (Ok(sent), Ok(stat)) if stat.frames == *sent && stat.keyframe => {
                println!(
                    "RESULT=OK екран→H.264→AEAD(PAKE-ключ)→сесія→розшифр.→валідація: надіслано {sent}, прийнято {} ({} байт H.264), keyframe(SPS)=так",
